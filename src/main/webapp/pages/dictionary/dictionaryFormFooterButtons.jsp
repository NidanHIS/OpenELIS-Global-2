...
org.openelisglobal.internationalization.MessageUtil" %>
  <%@ page isELIgnored="false" %>
            ...
            <%! String idSeparator=ConfigurationProperties.getInstance().getPropertyValue("default.idSeparator"); %>
              <%--bugzilla 2061-2063--%>
                <% String recordFrozenDisableEdits="false" ; if
                  ...
                  previousDisabled=(String)request.getAttribute(IActionConstants.PREVIOUS_DISABLED); } if
                  (request.getAttribute(IActionConstants.NEXT_DISABLED) !=null) {
                  nextDisabled=(String)request.getAttribute(IActionConstants.NEXT_DISABLED); } String
                  saveDisabled=(String)request.getAttribute(IActionConstants.SAVE_DISABLED); if (saveDisabled=="false" )
                  { if (recordFrozenDisableEdits=="true" ) { saveDisabled="true" ; } } %>
   ...
   %>
      ...
      strHTML += '<body onBlur="fnHandleFocus();" onLoad="fnHandleFocus();" ><form name="confirmSaveIt" method="get" action=""><div id="popupBody"><tr><td><td class="popuplistdata">';
      strHTML += message;
      if (direction == 'next') {
      ...
      strHTML += '</td></table></table></form></body></html>';
  ...
  // ========== MODIFIED saveIt function with Yup validation ==========
  async function saveIt(form) {
    // Call Yup validation function defined in dictionary.jsp
    if (typeof validateDictionaryForm === 'function') {
      ...
      // Fallback in case validation function is missing (should not happen)
                        ...
                        <tr>
                          <td>
                            <button type="button"
                              ...
                              id="save" <%if (Boolean.valueOf(saveDisabled).booleanValue()) { %>
                              disabled="disabled" <% } %> >
                          ...
                          <td>
                            <button type="button" onclick="previousAction(document.getElementById('mainForm'), '');"
                              id="previous" <%if (Boolean.valueOf(previousDisabled).booleanValue()) { %>
                              disabled="disabled" <% } %> >
                          ...
                          <td>
                            <button type="button" onclick="nextAction(document.getElementById('mainForm'), '');"
                              id="next" <%if (Boolean.valueOf(nextDisabled).booleanValue()) { %>
                              disabled="disabled" <% } %> >
