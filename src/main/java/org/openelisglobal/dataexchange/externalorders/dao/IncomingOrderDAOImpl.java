...
public class IncomingOrderDAOImpl extends BaseDAOImpl<IncomingOrder, Integer> implements IncomingOrderDAO {
    ...
    public List<IncomingOrder> getPagedOrders(Timestamp from, Timestamp to, String search, int offset, int limit) {
        ...
        if (search != null && !search.trim().isEmpty()) {
            ...
            hql.append(" and (lower(e.externalOrderNumber) like :search"
                    + " or exists ("
                    + "select pi.id from PatientIdentity pi, Patient p, PatientIdentityType pit"
                    + " join p.person per"
                    + " where pi.identityData = e.patientGuid"
                    + " and pi.identityTypeId = pit.id"
                    + " and pit.identityType = 'GUID'"
                    + " and pi.patientId = p.id"
                    + " and (lower(p.nationalId) like :search"
                    + " or lower(per.lastName) like :search"
    ...
    public long countOrders(Timestamp from, Timestamp to, String search) {
        ...
        if (search != null && !search.trim().isEmpty()) {
            // Identical search clause as getPagedOrders — keeps count consistent with results.
            hql.append(" and (lower(e.externalOrderNumber) like :search"
                    + " or exists ("
                    + "select pi.id from PatientIdentity pi, Patient p, PatientIdentityType pit"
                    + " join p.person per"
