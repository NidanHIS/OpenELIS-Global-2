 ...
 *
 * Strategy: the service is mocked so every test is purely about the
 * controller + validator pipeline — no DB, no Spring context overhead
 * beyond what BaseWebContextSensitiveTest already provides.
 *
 * Coverage:
 *  - Happy path: test upsert, panel upsert
 *  - Validation: missing identifier, missing name, LOINC too long,
 *    missing resultType, unknown resultType, panel missing sampleType
 *  - Multiple errors collected in one shot
 *  - Service-level CatalogValidationException bubbles to 400
 *  - Unexpected service exception → 500 with JSON body (no raw string)
 *  - Response envelope shape: status, message, guid, errors fields
 */
...
public class CatalogInboundRestControllerTest extends BaseWebContextSensitiveTest {
    ...
    public void testValidation_multipleErrors_allReturnedAtOnce() throws Exception {
        ...
        mockMvc.perform(post("/rest/catalog")
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.status").value("VALIDATION_ERROR"))
                .andExpect(jsonPath("$.errors.length()").value(3)); // identifier ok (loinc present), name missing, loinc too long, resultType missing = 3
    ...
    public void testServiceValidationException_returns400() throws Exception {
        ...
        Mockito.when(mockService.upsert(Mockito.any(), Mockito.any()))
                .thenThrow(new org.openelisglobal.dataexchange.externalcatalog.exception
                        .CatalogValidationException("No valid sample types could be resolved"));
    ...
    public void testSuccessResponse_noErrorsField() throws Exception {
        ...
        mockMvc.perform(post("/rest/catalog")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.errors").doesNotExist())
                .andExpect(jsonPath("$.guid").value("envelope-001"));
    }
