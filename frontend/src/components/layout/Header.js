...
function OEHeader({
  ...
  // Only increments logoVersion when the URL actually changes to avoid
  // forcing the browser to re-fetch an unchanged image on every call.
  const loadHeaderLogo = () => {
    getBranding((response) => {
      if (response && response.headerLogoUrl) {
        setHeaderLogoUrl((prev) => {
          if (prev !== response.headerLogoUrl) {
            setLogoVersion((v) => v + 1);
          }
          return response.headerLogoUrl;
        });
      }
    });
  };

  // Load header logo exactly once on mount.
  // Auth changes do not affect the logo URL — the menu reloads on auth
  // change (see the menu useEffect above) but the logo does not need to.
  useEffect(() => {
    loadHeaderLogo();
  ...
  // Dependency array is [] so the listener is registered once on mount
  // and never torn down/re-registered on auth state changes.
  useEffect(() => {
  ...
  const logo = () => {
    ...
    // IMPORTANT: use process.env.PUBLIC_URL prefix for the fallback so the
    // path resolves correctly regardless of which route is active.
    // A bare relative path like "../images/..." resolves differently from
    // "/openelis/" vs "/openelis/SomePage" and causes an infinite onError
    // loop on the homepage because nginx serves index.html for the missing
    // path, the browser fails to parse it as an image, and onError fires
    // the same broken path again.
    const defaultLogoSrc = `${process.env.PUBLIC_URL}/images/openelis_logo.png`;
